...
class TargetFire(Ability):
    ...
    def draw(self, offset = pygame.Vector2(0, 0)) -> None:
        ...
        for t in self.selected_targets:
            pygame.draw.circle(self.scene.screen, self.color1, t.center_position + offset, t.custom_hitbox_size.x, width = 3) # draw a spinning rect or triangle
    ...
    def update(self, delta_time: float) -> None:
        ...
        if self.active:
            ...
            for e in self.scene.entities:
                if e.custom_hitbox.collidepoint(self.position) and "player" not in e.tags and e not in self.selected_targets:
