...
class Editor(Scene):
    ...
    def update(self) -> None:
        ...
        if self.path_selector is not None:
            if len(self.path_selector.points) == 0:
                pygame.draw.line(self.screen, (100, 0, 0), self.axis_points(self.selected_axis), self.client.mouse_pos)
            if len(self.path_selector.points) == 1:
                pygame.draw.line(self.screen, (100, 0, 0), self.path_selector.points[-1], self.client.mouse_pos)
            if len(self.path_selector.points) == 2:
        ...
        for b in self.buttons:
            ...
            if b.is_pressing_mousedown_instant(pygame.Vector2(self.client.mouse_pos) - self.menu_pos):
                b.execute()
